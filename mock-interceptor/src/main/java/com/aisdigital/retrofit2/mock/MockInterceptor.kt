        ...
        request?.url()?.query()?.let { query ->
            ...
            split.forEach { param ->
                ...
                if (paramSplit.size == 2)
                    queryList.add(ParamData(paramSplit[0], paramSplit[1]))
            }
        }
        request?.body()?.let { body ->
            if (body is FormBody) {
                for (i in 0 until body.size())
                    queryList.add(ParamData(body.name(i), body.value(i)))
            }
                        ...
                        for (element in split) {
                            ...
                            if(keyValueSplit.size == 2) {
                                ...
                                if(key == "name") {
                                    ...
                                    queryList.add(ParamData(multiparkParamKey, multiparkParamValue))
                                }
                            }
                        }
                    }
